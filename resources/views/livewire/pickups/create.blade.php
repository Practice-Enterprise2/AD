  ...
  <form wire:submit.prevent="save" class="grid gap-4 gap-y-2 grid-cols-1 lg:grid-cols-5 lg:max-w-5xl w-10/12">
    ...
    <div class="lg:col-span-5 py-2">
      ...
      <select name="shipment" wire:model.defer="pickup_shipment_id" class="w-full">
        ...
        <option value="{{ $shipment->id }}" {{ $pickup_shipment_id !==null && $pickup_shipment_id===$shipment->id ?
          'selected="selected"' : '' }}>{{ $shipment->source_address->city }} &rsaquo; {{
    ...
    <div class="lg:col-span-1">
      ...
      <input id="house_number" name="house_number" class="w-full" wire:model.defer="house_number" type="text" required>
      @error('house_number') <span class="text-red-500">{{ $message }}</span> @enderror
    </div>
    <div class="lg:col-span-2">
      <label for="city">City</label>
      <input id="city" name="city" class="w-full" wire:model.defer="city" type="text" required>
      @error('city') <span class="text-red-500">{{ $message }}</span> @enderror
    </div>
    <div class="lg:col-span-1">
      <label for="postal_code">Postal code</label>
      <input id="postal_code" name="postal_code" class="w-full" wire:model.defer="postal_code" type="text" required>
      @error('postal_code') <span class="text-red-500">{{ $message }}</span> @enderror
    </div>
    <div class="lg:col-span-2">
      <label for="region">Region</label>
      <input id="region" name="region" class="w-full" wire:model.defer="region" type="text" required>
      @error('region') <span class="text-red-500">{{ $message }}</span> @enderror
    </div>
    <div class="lg:col-span-5">
      <label for="country">Country</label>
      <input id="country" name="country" class="w-full" wire:model.defer="country" type="text" required>
      @error('country') <span class="text-red-500">{{ $message }}</span> @enderror
    </div>
    <div class="lg:col-span-5">
      <label for="time">Time</label>
      <input id="time" name="time" class="w-full" wire:model.defer="time" type="datetime-local" required>
