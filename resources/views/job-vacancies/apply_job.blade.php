    ...
    <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
      <div class="overflow-hidden bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg">
        <div class="p-6 text-gray-900 dark:text-gray-100">
          ...
          <form method="post" action="{{ route('JobVacanciesController.apply') }}">
            @csrf
                <input type="hidden" name="job_id" id="job_id" value="{{$job}}">
                <div class="mb-2 grid gap-2 md:grid-cols-2">
                    <x-text-input id="applicant_name" class="mt-1 block w-full p-[10px]"
                    type="text" name="applicant_name" :value="old('applicant_name')"
                    placeholder="name*" required />
                    <x-text-input id="applicant_email" class="mt-1 block w-full p-[10px]"
                    type="email" name="applicant_email" :value="old('applicant_email')"
